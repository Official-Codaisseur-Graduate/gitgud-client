...
const spacecamel =(string) => {
  const stringToLowerCase = string.replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase();
...
export default function ProfileStats(props) { 
  ...
  {props.user && props.user.profileStats && Object.entries(props.user.profileStats).map(([key, value]) => {
    if (key !== "__typename") { 
    return (
      <div className="stats__profile-item" key={key}>
        {value ? <img src={require("../img/checked.svg")} alt='checked' width="30" height="30"/> : <img src={require("../img/cancel.svg")} alt='not-checked' width="30" height="30"/>}
  ...
  <section className="stats__average">
    ...
    <p> Total pinned repositories: {props.user.stats.totalPinnedRepros} </p>
